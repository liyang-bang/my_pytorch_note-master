...
def main():
    ...
    # 超参数可自行指定
    model = train(train_dataset, val_dataset, batch_size=64, epochs=200, learning_rate=0.1, wt_decay=0)
    # 保存模型
    torch.save(model, 'D:/Dataset/amap_traffic_GaoDe/cnn_model/model_net6.pkl')
    torch.save(model.state_dict(), 'D:/Dataset/amap_traffic_GaoDe/cnn_model/model_net6.pt')
