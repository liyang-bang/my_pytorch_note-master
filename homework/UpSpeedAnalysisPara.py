...
def process(dfAll,speedFilter,baseFile) :
    outJpgPath = baseFile + 'jpg-'+speedFilter+'/'
    ...
    saveRoidPath = saveRoidPathF+speedFilter+'_rateUpSpeed.csv'
    # print(dfAll.head())
...
def main():
    ...
    baseFile = sys.argv[2]+ '/'
    readFile = sys.argv[3]
    readFile1 = sys.argv[4]
    roadClass = sys.argv[5]
    ...
    names = cols = ['car_ff', 'truck_ff', 'min_key', 'linkID','roadClass','vehicle_type', 'vehicle_num', 'speed_sample', 'key']
    # if readFile != '00':
    ...
    #过滤速度<40的数据 拥堵
    print('fliter speed <'+classRange[0]+' start ')
    dfAll35lower = dfAll[((dfAll['vehicle_type'] == 1) & (dfAll['speed_sample'] < classRange[0])) | (dfAll['vehicle_type'] == 0)]
    process(dfAll35lower, '0-'+classRange[0],baseFile)
    print('fliter speed <='+classRange[0]+'  end ')


    print('fliter speed >' + classRange[0] + ' and speed <='+classRange[1]+ ' start ')
    dfAll35lower = dfAll[((dfAll['vehicle_type'] == 1) & (dfAll['speed_sample'] >= classRange[0]) & (dfAll['speed_sample'] < classRange[1])) | (dfAll['vehicle_type'] == 0)]
    process(dfAll35lower, classRange[0]+'-'+classRange[1], baseFile)
    print('fliter speed >' + classRange[0] + ' and speed <='+classRange[1]+  ' end ')


    #过滤速度>35的数据
    print('fliter speed >'+classRange[1]+' start ')
    dfAll35lower = dfAll[((dfAll['vehicle_type'] == 1) & (dfAll['speed_sample'] >= classRange[1])) | (dfAll['vehicle_type'] == 0)]
    process(dfAll35lower, classRange[1],baseFile)
    print('fliter speed >'+classRange[1]+' end ')
